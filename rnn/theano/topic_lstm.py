...
class DirichletTransition(object):
    ...
    def __call__(self, X):
        ...
        I = T.shape_padleft(T.ones((A.shape[1],1))*T.shape_padleft(self.w_init))
        A = T.exp(T.concatenate([I, A], axis=0))
        B = T.sum(T.gammaln(A), axis=-1) - T.gammaln(T.sum(A, axis=-1))
        L = T.sum((A-1)*T.log(X), axis=-1) - B
        return L
